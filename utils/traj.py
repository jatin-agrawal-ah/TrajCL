...
def large_time_shift(src, time_indices=None):
    ...
    new_time_indices = [(t + time_shift_val)% Config.traj_max_time for t in time_indices]
...
def get_aug_fn(name: str):
    return {'straight': straight, 'simplify': simplify, 'shift': shift,
            'mask': mask, 'subset': subset, 'reverse': reverse, 'large_time_shift': large_time_shift}.get(name, None)
...
#     for i in range(1, len(src) - 1):
#         dist = (lens[i-1] + lens[i]) / 2
#         velocity = dist*2 / ((time_indices[i+1] - time_indices[i-1])*600) if (time_indices[i+1] - time_indices[i-1]) > 0 else 0
#         dist = dist / (Config.trajcl_local_mask_sidelen / 1.414) # float_ceil(sqrt(2))
...
#         x = (src[i][0] - cs.x_min) / (cs.x_max - cs.x_min)
#         y = (src[i][1] - cs.y_min)/ (cs.y_max - cs.y_min)
#         tgt.append( [x, y, dist, radian, velocity] )

#     x = (src[0][0] - cs.x_min) / (cs.x_max - cs.x_min)
#     y = (src[0][1] - cs.y_min)/ (cs.y_max - cs.y_min)
#     velocity = lens[0]/ ((time_indices[1] - time_indices[0])*600) if (time_indices[1] - time_indices[0]) > 0 else 0
#     tgt.insert(0, [x, y, 0.0, 0.0, velocity] )

#     x = (src[-1][0] - cs.x_min) / (cs.x_max - cs.x_min)
#     y = (src[-1][1] - cs.y_min)/ (cs.y_max - cs.y_min)
#     velocity = lens[-1]/ ((time_indices[-1] - time_indices[-2])*600) if (time_indices[-1] - time_indices[-2]) > 0 else 0
#     tgt.append( [x, y, 0.0, 0.0, velocity] )
#     # tgt = [length, 5]
#     return tgt
